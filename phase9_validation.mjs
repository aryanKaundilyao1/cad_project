...
async function run() {
  console.log("=== STEP 6: PURGE INVALID SIGNALS ===");
  // We can just purge all from signal_instances, since it cascades to opportunity_signals
  const { error: delErr } = // skipped purge.neq('id', '00000000-0000-0000-0000-000000000000');
  if (delErr) { console.error("Error purging:", delErr); return; }
  console.log("Purged invalid signals.");

  console.log("\n=== STEP 7: REGENERATE SIGNALS ===");
  // Fetch all opportunities
  const { data: opps, error: oppErr } = await supabase.from('opportunities').select('id');
  let successCount = 0;
  for (const opp of opps) {
    // skipped rpc('generate_google_maps_signals', { p_opportunity_id: opp.id });
    if (!error) successCount++;
  }
  console.log(`Generated signals for ${successCount} opportunities.`);
  ...
  console.log(`average signals per opportunity: ${(osCount / opps.length).toFixed(2)}`);
...
async function traceOpp(oppId, label) {
  ...
  if (sigErr) { console.error(sigErr); return; }
  if (!signals) return;
  
  const { data: signals, error: sigErr } = await supabase.from('opportunity_signals')
    .select('*, signal_instances(*, signal_registry(name)), signal_weights(weight)')
