...
// We will just rewrite the Navigation component to be much cleaner and simpler, removing everything requested.
const newNavContent = `import { Link, useNavigate, useLocation } from "react-router-dom";
...
const Navigation = () => {
  ...
  const getUserInitial = () => {
    const name = profile?.full_name || profile?.company_name || user?.email || "";
  ...
  const NavDropdown = ({ title, items }: { title: string, items: { label: string, to: string }[] }) => (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-48 bg-card/95 backdrop-blur-xl border-white/10">
        {items.map((item, idx) => (
          <DropdownMenuItem 
            key={idx} 
      ...
      <div className="container mx-auto px-4">
        <div className="flex h-16 items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-2 shrink-0 group">
            <span className="text-lg font-bold tracking-tight text-foreground group-hover:text-glow transition-all duration-300">
              <span className="text-gradient-blue">JAS</span>{" "}
            ...
            {user ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-white/5 transition-colors focus:outline-none">
                ...
                <DropdownMenuContent align="end" className="w-56 bg-card/95 backdrop-blur-xl border-white/10">
                  <div className="px-3 py-2.5 border-b border-white/5">
                    <p className="text-sm font-medium">{profile?.full_name || profile?.company_name || user.email}</p>
                  ...
                  <DropdownMenuSeparator className="bg-white/5" />
                  <DropdownMenuItem onSelect={(e) => { e.preventDefault(); handleSignOut(); }} className="gap-2 cursor-pointer text-destructive"><LogOut className="h-4 w-4" /> Sign Out</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            ) : (
              <>
                ...
                <Button size="sm" className="text-[13px] bg-primary text-primary-foreground hover:bg-primary/90 shadow-[0_0_15px_rgba(34,211,238,0.2)] border-0" onClick={() => navigate('/auth')}>
                  Start Free Trial
                </Button>
              </>
            )}
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
              <SheetTrigger asChild>
                <button className="p-2 hover:bg-white/5 rounded-lg transition-colors"><Menu className="h-5 w-5 text-foreground/70" /></button>
              </SheetTrigger>
              <SheetContent side="right" className="w-80 p-0 bg-card/95 backdrop-blur-xl border-white/10">
                <SheetHeader className="px-6 pt-6 pb-4 border-b border-white/5">
                  <SheetTitle className="text-left font-bold"><span className="text-gradient-blue">JAS</span> CONNECT</SheetTitle>
                </SheetHeader>
                  ...
                  <div className="border-t border-white/5 px-4 py-4 space-y-2">
                    {user ? (
                      <>
                        ...
                        <Link to="/workspace" onClick={() => setMobileOpen(false)}><Button variant="outline" className="w-full justify-start gap-2 border-white/10" size="sm"><Database className="h-4 w-4" /> Dashboard</Button></Link>
...
fs.writeFileSync(navPath, newNavContent);
