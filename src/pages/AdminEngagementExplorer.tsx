...
import { EngagementScoreService } from "@/services/intelligence/scoring/engagement/EngagementScoreService";
import { EngagementFeatureSnapshotService } from "@/services/intelligence/scoring/engagement/EngagementFeatureSnapshotService";
import { Loader2, RefreshCcw, Users, Info, Clock, CheckCircle2, ShieldAlert } from "lucide-react";

const AdminEngagementExplorer = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [scoreRes, featRes, auditRes, histRes] = await Promise.all([
        EngagementScoreService.getEngagementScore(selectedCompanyId, selectedProductId),
        EngagementFeatureSnapshotService.getSnapshots(selectedCompanyId, selectedProductId),
        EngagementScoreService.getAuditLog(selectedCompanyId, selectedProductId),
        EngagementScoreService.getEngagementHistory(selectedCompanyId, selectedProductId)
      ]);
  ...
  const handleRebuild = async () => {
    ...
    try {
      ...
      await EngagementScoreService.calculateEngagementScore(selectedCompanyId, selectedProductId, 'Admin Manual Rebuild');
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold flex items-center gap-2">
              ...
              Engagement Score Explorer
            </h1>
            <p className="text-gray-500 mt-2">Analyze stakeholder count and seniority (Max 20 pts).</p>
          </div>
        ...
        {isLoading ? (
          <div className="flex justify-center p-12"><Loader2 className="h-12 w-12 animate-spin text-purple-600" /></div>
        ) : (!selectedCompanyId || !selectedProductId) ? (
          <div className="text-center p-12 text-gray-500">Select a company and product to view the score.</div>
        ) : !scoreData || scoreData.engagement_score === null ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-6 lg:col-span-1">
              <Card className="bg-purple-50 border-purple-100">
                <CardHeader>
                  <CardTitle className="text-purple-800 flex justify-between">
                    Engagement Score
                    <Badge variant="outline" className="bg-white">{scoreData.engagement_version}</Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-6xl font-black text-purple-600 mb-2">
                    {scoreData.engagement_score} <span className="text-xl text-purple-300">/ 20</span>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    {(!scoreData.score_reason_codes || scoreData.score_reason_codes.length === 0) && (
                      <div className="text-sm text-gray-500">No active engagement found.</div>
                    )}
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="lg:col-span-2 space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>The frozen engagement signals used to calculate the current score.</CardDescription>
                </CardHeader>
                <CardContent>
                  <Table>
                    <TableHeader>
                      ...
                      {auditLog.map((log: any) => (
                        <TableRow key={log.id}>
                          ...
                          <TableCell className="text-sm font-medium text-purple-600">
                            {log.old_score ? `${log.old_score.engagement_score} → ${log.new_score.engagement_score}` : `New: ${log.new_score.engagement_score}`}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
...
export default AdminEngagementExplorer;
