...
import { LeadQualityAnalyzer } from './LeadQualityAnalyzer';
...
export class COIFEngine {
  public static evaluate(client: ClientContext, leads: Lead[]): COIFResult[] {
    const results: COIFResult[] = leads.map(lead => {
      ...
      // 6. Lead Quality & Digital Presence (Critical for differentiation within same source)
      const leadQualityAnalysis = LeadQualityAnalyzer.analyze(lead);
      reasons = reasons.concat(leadQualityAnalysis.reasons);

      // Calculate Raw COIF Score
      // Product Match (40%), Client Context (30%), Lead Quality (30%)
      const rawCOIF = (productAnalysis.score * 0.4) + (clientAnalysis.score * 0.3) + (leadQualityAnalysis.score * 0.3);
