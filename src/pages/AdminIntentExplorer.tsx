...
import { IntentScoreService } from "@/services/intelligence/scoring/intent/IntentScoreService";
import { IntentFeatureSnapshotService } from "@/services/intelligence/scoring/intent/IntentFeatureSnapshotService";
import { Loader2, RefreshCcw, Activity, Info, Clock, CheckCircle2, ShieldAlert } from "lucide-react";

const AdminIntentExplorer = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [scoreRes, featRes, auditRes, histRes] = await Promise.all([
        IntentScoreService.getIntentScore(selectedCompanyId, selectedProductId),
        IntentFeatureSnapshotService.getSnapshots(selectedCompanyId, selectedProductId),
        IntentScoreService.getAuditLog(selectedCompanyId, selectedProductId),
        IntentScoreService.getIntentHistory(selectedCompanyId, selectedProductId)
      ]);
  ...
  const handleRebuild = async () => {
    ...
    try {
      ...
      await IntentScoreService.calculateIntentScore(selectedCompanyId, selectedProductId, 'Admin Manual Rebuild');
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold flex items-center gap-2">
              ...
              Intent Score Explorer
            </h1>
            <p className="text-gray-500 mt-2">Analyze research behavior and intent signals (Max 30 pts).</p>
          </div>
        ...
        {isLoading ? (
          <div className="flex justify-center p-12"><Loader2 className="h-12 w-12 animate-spin text-blue-600" /></div>
        ) : (!selectedCompanyId || !selectedProductId) ? (
          <div className="text-center p-12 text-gray-500">Select a company and product to view the score.</div>
        ) : !scoreData || scoreData.intent_score === null ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-6 lg:col-span-1">
              <Card className="bg-blue-50 border-blue-100">
                <CardHeader>
                  <CardTitle className="text-blue-800 flex justify-between">
                    Intent Score
                    <Badge variant="outline" className="bg-white">{scoreData.intent_version}</Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-6xl font-black text-blue-600 mb-2">
                    {scoreData.intent_score} <span className="text-xl text-blue-300">/ 30</span>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    {(!scoreData.score_reason_codes || scoreData.score_reason_codes.length === 0) && (
                      <div className="text-sm text-gray-500">No active intent signals found.</div>
                    )}
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="lg:col-span-2 space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>The frozen intent signals used to calculate the current score.</CardDescription>
                </CardHeader>
                <CardContent>
                  <Table>
                    <TableHeader>
                        ...
                        <TableRow key={f.id}>
                          ...
                          <TableCell className="text-sm">
                            {typeof f.feature_value === 'object' && f.feature_value !== null 
                              ...
                              : (f.feature_value || <span className="text-gray-400 italic">0</span>)}
                          </TableCell>
                      ...
                      {auditLog.map((log: any) => (
                        <TableRow key={log.id}>
                          ...
                          <TableCell className="text-sm font-medium text-blue-600">
                            {log.old_score ? `${log.old_score.intent_score} → ${log.new_score.intent_score}` : `New: ${log.new_score.intent_score}`}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
...
export default AdminIntentExplorer;
